...
public:
	...
	void SetInputLayout(D3D12_INPUT_LAYOUT_DESC layout);       // 入力レイアウトの設定
	void SetRootSignature(ID3D12RootSignature* rootSignature); // ルートシグネチャの設定
	void SetVS(std::wstring filePath);                         // 頂点シェーダーの設定
	void SetPS(std::wstring filePath);                         // ピクセルシェーダーの設定
	void Create();                                             // パイプラインステートの生成
